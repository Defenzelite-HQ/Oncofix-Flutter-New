...
Future<void> generateService() async {
  ...
  String _serviceFile = modulrService.stub
      .replaceAll('{MODULE}', Utils.ucFirst(serviceName, preserveAfter: true));
  ...
  /// Write File
  Utils.writeFile(
      "$servicePath/${Utils.ucFirst(serviceName, preserveAfter: true)}Service.dart",
      _serviceFile);
  Utils.writeFile(
      "$servicePath/Mock${Utils.ucFirst(serviceName, preserveAfter: true)}Service.dart",
      _serviceMockFile);
  Utils.writeFile(
      "$servicePath/App${Utils.ucFirst(serviceName, preserveAfter: true)}Service.dart",
      _serviceAppFile);

  /// Show Success message
  print(green(
      '"$servicePath/${Utils.ucFirst(serviceName, preserveAfter: true)}Service.dart" generated successfully.'));
  print(green(
      '"$servicePath/Mock${Utils.ucFirst(serviceName, preserveAfter: true)}Service.dart" generated successfully.'));
  print(green(
      '"$servicePath/App${Utils.ucFirst(serviceName, preserveAfter: true)}Service.dart" generated successfully.'));
}
...
bool _validateArgs(List<String> args) {
  ...
  if (moduleName == "") {
    print(red(
        'Please provide module name to generate the service\nExample [--on=<module_name>]'));
    return false;
  }
